                                     ...
                                     const char * template_locale)
{
     ...
     * it can be restored to later */
    restore_to_locale =
                      stdize_locale(savepv(raw_querylocale_r(switch_category)));
    ...
    if (template_locale == NULL) {
        template_locale = per_thread_querylocale_r(template_category);
        if (! template_locale) {
            Perl_croak(aTHX_
             "panic: %s: %d: Could not find current %s locale, errno=%d\n",
...
#  endif
    ...
    save_input_locale = stdize_locale(savepv(raw_querylocale_r(category)));
