...
void
Perl_locale_panic(const char * msg, const char * file_name, const line_t line)
{
    dTHX;
...
/* In contrast, the other do_setlocale() macros are our added layers upon the base
...
#  define raw_setlocale_c(cat, locale)  porcelain_setlocale(cat, locale)
#  define raw_setlocale_i(i, locale)    raw_setlocale_c(categories[i], locale)
#  define raw_setlocale_r(cat, locale)  raw_setlocale_c(cat, locale)

#  define per_thread_setlocale_c(cat, locale)                               \
                                        porcelain_setlocale(cat, locale)
#  define per_thread_setlocale_i(i, locale)                                 \
                            per_thread_setlocale_c(categories[i], locale)
#  define per_thread_setlocale_r(cat, locale)                               \
                                       per_thread_setlocale_c(cat, locale)

#  define void_setlocale_c(cat, locale)                                     \
    STMT_START {                                                            \
        if (! porcelain_setlocale(cat, locale)) {                           \
...
#  define void_setlocale_i(i, locale)                                       \
                                  void_setlocale_c(categories[i], locale)
...
#  define raw_querylocale_c(cat)        porcelain_setlocale(cat, NULL)
#  define raw_querylocale_r(cat)        raw_querylocale_c(cat)
#  define raw_querylocale_i(i)          raw_querylocale_c(categories[i])

#  define per_thread_querylocale_c(cat) raw_querylocale_c(cat)
...
#    define raw_setlocale_i(i, locale)  common_emulate_setlocale(i, locale)
#    define raw_setlocale_c(cat, locale)                                    \
                                      raw_setlocale_i(cat##_INDEX_, locale)
#    define raw_setlocale_r(cat, locale)                                    \
                   raw_setlocale_i(get_category_index(cat, locale), locale)

#    define per_thread_setlocale_i(i, locale)                               \
                                        common_emulate_setlocale(i, locale)
#    define per_thread_setlocale_c(cat, locale)                             \
                               per_thread_setlocale_i(cat##_INDEX_, locale)
#    define per_thread_setlocale_r(cat, locale)                             \
            per_thread_setlocale_i(get_category_index(cat, locale), locale)
...
#    define bool_setlocale_r(cat, locale)                                \
             bool_setlocale_i(get_category_index(cat, locale), locale)

#  define raw_querylocale_i(i)          my_querylocale_i(i)
#  define raw_querylocale_c(cat)        raw_querylocale_i(cat##_INDEX_)
#  define raw_querylocale_r(cat)                                            \
                           raw_querylocale_i(get_category_index(cat,NULL))

#  define per_thread_querylocale_i(i)   my_querylocale_i(i)
#  define per_thread_querylocale_c(cat)                                     \
                                    per_thread_querylocale_i(cat##_INDEX_)
...
STATIC const char *
S_my_querylocale_i(const unsigned int index)
{
    ...
    assert(inRANGE(index, 0, NOMINAL_LC_ALL_INDEX));
...
S_setlocale_from_aggregate_LC_ALL(pTHX_ const char * locale, const line_t line)
{
    ...
    const char * name_end;
    const char * locale_on_entry = savepv(per_thread_querylocale_c(LC_ALL));
    const char * retval;
...
S_find_locale_from_environment(pTHX_ const unsigned int index)
{
     ...
     * from the standard documented value, but khw believes it is better to
     * know what's going on, even if different from native, than to just guess
     *
                      ...
                      const line_t line)
{
    ...
    if (UNLIKELY(index > NOMINAL_LC_ALL_INDEX)) { /* Out-of bounds */
        return NULL;
    }
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
        "(%d): emulate_setlocale_i input=%d (%s), mask=0x%x, \"%s\", index=%d\n",
        line, categories[index], category_names[index],
        mask, new_locale, index));
                ...
                for (i = 0; i < NOMINAL_LC_ALL_INDEX; i++) {
                    update_PL_curlocales_i(i,
                                           porcelain_setlocale(categories[i],
                                                               NULL),
                                           cBOOL(i == NOMINAL_LC_ALL_INDEX - 1));
                }
            }
...
#  ifdef USE_QUERYLOCALE
    ...
    if (strEQ(new_locale, "")) {
        new_locale = querylocale_l(index, new_obj);
    }
                            ...
                            const line_t caller_1_line)
{
    ...
    if (current == NULL) {
        current = raw_querylocale_i(cat_index);
    }
...
S_new_ctype(pTHX_ const char *newctype)
{
    ...
    save_newctype = stdize_locale(savepv(newctype));
...
S_new_LC_ALL(pTHX_ const char *unused)
{
    ...
    for (i = 0; i < NOMINAL_LC_ALL_INDEX; i++) {
        if (update_functions[i]) {
            update_functions[i](aTHX_ per_thread_querylocale_i(i));
        }
    }
}
...
#  endif
        ...
        retval = per_thread_querylocale_r(category);
...
#  endif
    ...
    retval = raw_querylocale_i(cat_index);
...
#  define UTF8NESS_UNKNOWN  -1 /* mnemonic value for 'known_utf8' */
    ...
    assert(inRANGE(cat_index, 0, NOMINAL_LC_ALL_INDEX));
...
#  if defined(HAS_SOME_LANGINFO) || defined(HAS_MBTOWC) || defined(HAS_MBRTOWC)
     ...
     * logical 'locale'.  This is because 'locale' isn't necessarily readily
     * available, and this way we only need to look it up in this more unlikely
     * case */

    if (locale == NULL) {
        locale = per_thread_querylocale_i(cat_index);
    }
...
#    ifdef USE_LOCALE_MONETARY
        ...
        monetary_locale_is_utf8 = is_locale_utf8(per_thread_querylocale_c(LC_MONETARY));
...
#  endif
        ...
        locale = raw_querylocale_i(cat_index);
        while (strings->name) {
                                    ...
                                    const int unused)
{
    ...
    switch (item) {
      case CRNCYSTR:
        ...
        /* khw couldn't figure out how to find out if the $ should replace the
         * radix.  So those rare locales that have that won't get the proper
         * specificaton. */
        break;
...
Perl_langinfo8(const nl_item item, int * utf8ness)
{
 ...
 && (defined(HAS_SOME_LANGINFO) || defined(HAS_SOME_LOCALECONV))
      ...
      /* These LC_TIME items require nl_langinfo() */
 ...
 && (defined(HAS_SOME_LANGINFO) || defined(HAS_STRFTIME))

      /* The rest are recoverable if we have either nl_langinfo() or strftime()
       * */
      default:
        cat_index = LC_TIME_INDEX_;
        break;

#else   /* Below we either have no LC_TIME, or no way to calculate these */
                ...
                int * utf8ness)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                           "Entering nl_langinfo item=%d, using ", item);
 ...
 * Configuration */
    ...
    if (locale == USE_UNDERLYING_LOCALE) {
        ...
        if (strNE(want_locale, per_thread_querylocale_i(cat_index))) {
...
#    else

    {
        bool done = FALSE;
        if (lang) {
            sl_result[LC_ALL_INDEX_] = porcelain_setlocale(LC_ALL, setlocale_init);
        ...
        if (! setlocale_failure) {
            ...
            for (i = 0; i < LC_ALL_INDEX_; i++) {
                locale_param = (   ! done
                            ...
                            ? setlocale_init
                            : NULL;
                sl_result[i] = porcelain_setlocale(categories[i], locale_param);
                if (! sl_result[i]) {
                    setlocale_failure = TRUE;
                }
...
#    ifdef WIN32    /* Note that assumes Win32 has LC_ALL */
            ...
            if (strEQ(trial_locale, "")) {
                ...
                system_default_locale = porcelain_setlocale(LC_ALL, "");
...
#  ifdef LC_ALL
        ...
        sl_result[LC_ALL_INDEX_] = porcelain_setlocale(LC_ALL, trial_locale);
...
#  endif /* LC_ALL */
        ...
        if (! setlocale_failure) {
            ...
            for (j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
                curlocales[j] = porcelain_setlocale(categories[j], trial_locale);
                if (! curlocales[j]) {
                    setlocale_failure = TRUE;
                }
    ...
    if (ok < 1) {   /* If we tried to fallback */
        ...
        if (! setlocale_failure) {  /* fallback succeeded */
           msg = "Falling back to";
            ...
            for (j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
                Safefree(curlocales[j]);
                curlocales[j] = savepv(porcelain_setlocale(categories[j], NULL));
...
#  ifdef USE_POSIX_2008_LOCALE
    ...
    /* The porcelain setlocales haven't affected the P2008 locales.  Initialize
...
#  endif
                    ...
                    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                        "slope is now %zu; was %zu, base is now %zu; was %zu\n",
...
S_toggle_locale_i(pTHX_ const unsigned cat_index, const char * new_locale)
{
    ...
    locale_to_restore_to = raw_querylocale_i(cat_index);
...
Perl_my_strerror(pTHX_ const int errnum, int * utf8ness)
{
    ...
    if (! IN_LC(LC_MESSAGES)) {    /* Use C if not within locale scope */
        ...
        *utf8ness = 1;
               ...
               matches */
        const locale_t cur = duplocale(use_curlocale_scratch());

        errstr = savepv(strerror_l(errnum,
                                   newlocale(LC_CTYPE_MASK,
                                          per_thread_querylocale_c(LC_MESSAGES),
                                          cur)));
