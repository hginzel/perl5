...
#  endif    /* End of has LC_NUMERIC */
        ...
        retval = save_to_buffer(raw_querylocale_r(category),
...
#    else

    {
        bool done = FALSE;
        if (lang) {
            sl_result[LC_ALL_INDEX_] = raw_setlocale_c(LC_ALL, setlocale_init);
        ...
        if (! setlocale_failure) {
            ...
            for (i = 0; i < LC_ALL_INDEX_; i++) {
                locale_param = (   ! done
                            ...
                            ? setlocale_init
                            : NULL;
                sl_result[i] = raw_setlocale_i(i, locale_param);
                if (! sl_result[i]) {
                    setlocale_failure = TRUE;
                }
...
#    ifdef WIN32    /* Note that assumes Win32 has LC_ALL */
            ...
            if (strEQ(trial_locale, "")) {
                ...
                system_default_locale = raw_setlocale_c(LC_ALL, "");
...
#  ifdef LC_ALL
        ...
        sl_result[LC_ALL_INDEX_] = raw_setlocale_c(LC_ALL, trial_locale);
...
#  endif /* LC_ALL */
        ...
        if (! setlocale_failure) {
            ...
            for (j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
                curlocales[j] = raw_setlocale_i(j, trial_locale);
                if (! curlocales[j]) {
                    setlocale_failure = TRUE;
                }
    ...
    if (ok < 1) {   /* If we tried to fallback */
        ...
        if (! setlocale_failure) {  /* fallback succeeded */
           msg = "Falling back to";
            ...
            for (j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
                Safefree(curlocales[j]);
                curlocales[j] = savepv(raw_querylocale_i(j));
